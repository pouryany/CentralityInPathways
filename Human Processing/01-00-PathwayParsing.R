...
if(!dir.exists("HUMAN_PATHWAYLIST/")){
    a <- CHRONOS::downloadKEGGPathwayList("hsa")
    pathview::download.kegg(pathway.id = a$Id, kegg.dir = "./HUMAN_PATHWAYLIST/")
...
paths.address  <- paste("HUMAN_PATHWAYLIST/hsa",a$Id,".xml",sep = "")
...
cancer.list <- read.table(file = 'human_data/allOnco_May2018.tsv',
                          sep = '\t', header = TRUE)
