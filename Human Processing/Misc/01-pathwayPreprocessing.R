...
# Cancer related genes From MSigDB
        hsap.gene.ref1   <- read.csv("data/msigdblist.csv")
...
for (i in 1:length(graphs.hsap)){
    ...
    beet.source.vec     <- sna::betweenness(hsap.mat, cmode = "directed")
    ...
    beet.sink.vec     <- sna::betweenness(t(hsap.mat), cmode = "directed")
    ...
    beet.und.vec     <- sna::betweenness((hsap.mat), cmode = "undirected")
